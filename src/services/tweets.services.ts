...
class TweetsService {
  ...
  async increaseTweetViews(tweet_id: string, user_id: string) {
    ...
    return result.value as WithId<{ guest_views: number; user_views: number; updated_at: Date }>;
  }
  ...
  async getTweetChildren({
          ...
          $addFields: {
            mentions: {
              $map: {
                ...
                as: 'mention',
                in: {
                  // chỗ này là lấy cái fields mà mình mong muốn trả về
                  _id: '$$mention._id',
                  name: '$$mention.name',
    ...
    // Chỗ này là muốn return documetn về cho client, Mà updateOne k có support trả về data sau khi update nên phải làm vậy
    ...
    return { tweets, totalDocuments, totalPages: Math.ceil(totalDocuments / limit) };
  }
  ...
  async getNewFeeds({ user_id, page, limit }: { user_id: string; page: number; limit: number }) {
    ...
    return {
      tweets,
      totalDocuments: total[0]?.total || 0,
